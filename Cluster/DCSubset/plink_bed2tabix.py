...
for j in range(1,23):
	...
	plinker = 'echo "plink --noweb --nonfounders --bfile ' + genodir + 'hutt.imputed.newck --chr ' + str(j) + ' --make-bed --out ' + genodir + 'ByChr/' + outname + '.chr' + str(j) + '; plink --bfile ' + genodir + 'ByChr/' + outname + '.chr' + str(j) + ' --recodeA --out ' + genodir + 'ByChr/' + outname + '.chr' + str(j) + '; touch ' + genodir + 'ByChr/chr' + str(j) + '.done" | qsub -l h_vmem=2g -o ~/dump/ -e ~/dump/'
	ifier(plinker)
...
for j in range(1,23):
	converter = 'echo "python /mnt/gluster/data/internal_supp/hutt_ipsc/DCsubset/Scripts/raw2txt.py ' + str(j) + ' ' + genodir + ' ' + outname + '" | qsub -l h_vmem=8g -o ~/dump/ -e ~/dump/'
	ifier(converter)
