...
```{r}
setwd("C:/Users/Courtney/Dropbox/LCL-iPSC/GEMMA eQTLs/Hutt iPSCs/Pvals")

library(plyr)
library(stringr)

#Get PC names
all.pvals <- list.files(path = "C:/Users/Courtney/Desktop/GEMMATest/",pattern=".imputed.1Mb.bonferroni.regressPCs.gemma.eqtls.txt",full.names=T)
pval.pcs.names = str_split(all.pvals,"[.]")
pval.pcs = c()
for(i in 1:length(pval.pcs.names)){
  pval.pcs[i] = str_split(pval.pcs.names[[i]][2],"PC")[[1]][2]
}
pval.pcs = names(pvals)
...
for(i in 1:length(pcs)){
  ...
  PC.pvals <- list.files(path = "C:/Users/Courtney/Desktop/GEMMATest/",pattern = paste("PC", j, ".imputed.1Mb.bonferroni.regressPCs.gemma.eqtls.txt", sep=""),full.names=T)
...
##Create Gene List for DAVID background
genes = read.table('PC 17 _eQTLResults.LocalFDR.txt')
genelist = genes[unique(genes$V1),]
write.table(genelist$V1, 'eQTLGeneList.txt', sep='\t', quote=F, row.names=F)
```
